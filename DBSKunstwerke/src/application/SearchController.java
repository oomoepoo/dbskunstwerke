...
public class SearchController implements Initializable{
	...
	public void showMuseumDetails(ActionEvent event) {
		...
		try {
			...
			MuseumsProfileController mcontroller = new MuseumsProfileController("test", museum);
	...
	public void showArtistDetails(ActionEvent event) {
		try {
			...
			Stage primaryStage = new Stage();
			FXMLLoader loader = new FXMLLoader();
			KuenstlerProfilController kpc = new KuenstlerProfilController("test", user.getUsername());
	...
	public void showArtworkDetails(ActionEvent event) {
		try {
			...
			Stage primaryStage = new Stage();
			FXMLLoader loader = new FXMLLoader();
			KunstwerkProfilController kpc = new KunstwerkProfilController(1, "test");
			loader.setController(kpc);
			loader.setLocation(url);
			Pane root = loader.load();
			Scene scene = new Scene(root);
			scene.getStylesheets().add(getClass().getResource("application.css").toExternalForm());
			primaryStage.setScene(scene);
			primaryStage.setTitle("DBS-Kunstwerke: ");
			primaryStage.show();
		} catch (IOException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
	...
	private ObservableList<Collection> getCollectionsfromSQL() {
		...
		try {
			...
			while(results.next()){
				...
				Collection sammlung=new Collection(sammlungID, sname, (String)null);
	...
	private ObservableList<Artwork> getArtworksfromSQL(){
		...
		try {
			...
			while(rs.next()){
				...
				artworks.add(new Artwork(name, creationyear, artstyle, (String)null));
			}
		} catch (SQLException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
